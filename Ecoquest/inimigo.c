...
void iniciar_bot_com_sprite(Bot* bot, float x, float y, const char* nome, const char* tipo, int cenario, const char* caminho_sprite) {
    ...
    bot->caminho_sprite_batalha = caminho_sprite; // Salva o caminho
    if (caminho_sprite) {
...
void iniciar_bot_com_area_eliptica(Bot* bot, float x, float y, const char* nome, const char* tipo, 
    ...
    printf("Bot %s limitado a area eliptica: Centro(%.0f, %.0f) RaioH=%.0f RaioV=%.0f\n", 
           nome, centro_x, centro_y, raio_h, raio_v);
}
...
static bool posicao_valida(const Bot* bot, float x, float y) {
    ...
    if (bot->area_restrita.tipo == AREA_ELIPTICA) {
        ...
        // Equação da elipse: (dx/a)² + (dy/b)² <= 1
        // Onde a = raio_horizontal e b = raio_vertical
        ...
        // Margem de segurança para não ultrapassar a borda
        float margem_seguranca = 0.95f; // 95% do raio máximo
...
void atualizar_bot(Bot* bot, float largura_mapa, float altura_mapa, float delta_time) {
    ...
    // Se tem área restrita, verifica se a nova posição é válida
    if (bot->area_restrita.tipo != AREA_NENHUMA) {
        if (posicao_valida(bot, novo_x, novo_y)) {
            bot->x = novo_x;
            bot->y = novo_y;
        } else {
            // Inverte direção ao tentar sair da área
            bot->direcao_x = -bot->direcao_x;
            bot->direcao_y = -bot->direcao_y;
            bot->tempo_mudanca = 2.5f; // Força mudança de direção em breve
        }
        return;
    }
    
    // Lógica padrão - limita pelo cenário
    float min_x = 0.0f, max_x = largura_mapa;
    float min_y = 0.0f, max_y = altura_mapa;
    
    switch (bot->cenario) {
        case 0: // CENARIO1 - Superior esquerdo
            min_x = 0.0f;
            max_x = 640.0f;
            min_y = 0.0f;
            max_y = 360.0f;
            break;
        case 1: // CENARIO2 - Inferior esquerdo
            min_x = 0.0f;
            max_x = 640.0f;
            min_y = 360.0f;
            max_y = 720.0f;
            break;
        case 2: // CENARIO3 - Superior direito
            min_x = 640.0f;
            max_x = 1280.0f;
            min_y = 0.0f;
            max_y = 360.0f;
            break;
        case 3: // CENARIO4 - Inferior direito
            min_x = 640.0f;
            max_x = 1280.0f;
            min_y = 360.0f;
            max_y = 720.0f;
    ...
    // Limita dentro dos limites do cenário
