...
void inicializar_debug(DebugContext* debug, ALLEGRO_FONT* fonte) {
    ...
    debug->mostrar_fps = true;  // FPS ativo por padrão
    ...
    DEBUG_LOG("Sistema de debug inicializado");
}

void destruir_debug(DebugContext* debug) {
    if (!debug) return;
    DEBUG_LOG("Sistema de debug finalizado");
}
...
void debug_desenhar_grid(int largura_mapa, int altura_mapa, int tamanho_celula, ALLEGRO_COLOR cor, float camera_x, float camera_y, float zoom) {
    ...
    for (int x = 0; x <= largura_mapa; x += tamanho_celula) {
        float x1 = (x - camera_x) * zoom;
        float y1 = (0 - camera_y) * zoom;
        float y2 = (altura_mapa - camera_y) * zoom;
        al_draw_line(x1, y1, x1, y2, cor, 1.0f);
    }
    
    // Linhas horizontais
    for (int y = 0; y <= altura_mapa; y += tamanho_celula) {
        float x1 = (0 - camera_x) * zoom;
        float y1 = (y - camera_y) * zoom;
        float x2 = (largura_mapa - camera_x) * zoom;
...
void debug_processar_teclas(DebugContext* debug, ALLEGRO_EVENT* evento) {
    ...
    switch (evento->keyboard.keycode) {
        case ALLEGRO_KEY_F1:
            ...
            DEBUG_PRINT("Hitboxes: %s", debug->mostrar_hitboxes ? "ON" : "OFF");
            break;
        case ALLEGRO_KEY_F2:
            debug->mostrar_areas_restritas = !debug->mostrar_areas_restritas;
            DEBUG_PRINT("Areas Restritas: %s", debug->mostrar_areas_restritas ? "ON" : "OFF");
            break;
        case ALLEGRO_KEY_F3:
            debug->mostrar_fps = !debug->mostrar_fps;
            DEBUG_PRINT("FPS: %s", debug->mostrar_fps ? "ON" : "OFF");
            break;
        case ALLEGRO_KEY_F4:
            debug->mostrar_info_bots = !debug->mostrar_info_bots;
            DEBUG_PRINT("Info Bots: %s", debug->mostrar_info_bots ? "ON" : "OFF");
            break;
        case ALLEGRO_KEY_F5:
            debug->mostrar_grid = !debug->mostrar_grid;
            DEBUG_PRINT("Grid: %s", debug->mostrar_grid ? "ON" : "OFF");
            break;
        case ALLEGRO_KEY_F6:
            debug->mostrar_coordenadas = !debug->mostrar_coordenadas;
            DEBUG_PRINT("Coordenadas: %s", debug->mostrar_coordenadas ? "ON" : "OFF");
            break;
    }
}
...
void debug_desenhar_painel_info(DebugContext* debug, float jogador_x, float jogador_y, int cenario_atual, int bots_ativos) {
    ...
    al_draw_text(debug->fonte_debug, al_map_rgb(255, 255, 255), 10, y_offset, 0, buffer);
    y_offset += 15;
    ...
    // Legendas
    al_draw_text(debug->fonte_debug, al_map_rgb(150, 150, 150), 10, y_offset, 0, "[F1] Hitboxes");
    y_offset += 12;
    al_draw_text(debug->fonte_debug, al_map_rgb(150, 150, 150), 10, y_offset, 0, "[F2] Areas");
    y_offset += 12;
    al_draw_text(debug->fonte_debug, al_map_rgb(150, 150, 150), 10, y_offset, 0, "[F3] FPS");
    y_offset += 12;
    al_draw_text(debug->fonte_debug, al_map_rgb(150, 150, 150), 10, y_offset, 0, "[F4] Info Bots");
    y_offset += 12;
