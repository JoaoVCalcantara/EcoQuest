...
void desenhar_menu_batalha(ALLEGRO_FONT* fonte, int opcao, ALLEGRO_DISPLAY* display) {
    ...
    // ---- Opções dentro do losango ----
    // Corrigido: agora a ordem é Alimentar, Estudar, Correr
    ...
    while (batalhando) {
        ...
        if (ev.type == ALLEGRO_EVENT_KEY_DOWN) {
            ...
            case ALLEGRO_KEY_ENTER:
                switch (opcao) {
                case 0: // Alimentar
                    printf("Opção selecionada: Alimentar\n");
                    animal->alimentado = true;
                    animal->experiencia += 10;
                    if (animal->experiencia >= 100) {
                        printf("🎉 O animal atingiu 100 de experiência!\n");
                        batalhando = false; // encerra batalha automaticamente
                    }
                    break;

                case 1: // Estudar
                    printf("Opção selecionada: Estudar\n");
